 ...
 * Version              : 3.4.0
 * Author               : Joerg Pfeifer - joerg.pfeifer egplusww.com
 */
(function () {
  "use strict";
  ...
  eg.initEventDispatcher = function (obj) {
    ...
    var registry = {},
      register = function (type, method, params, once, ob) {
        ...
        //console.info("register", "type:", type, "registry[type]:", registry[type]);
    ...
    Object.defineProperty(obj, "dispatchEvent", {
      value: function (event) {
        ...
        if (registry.hasOwnProperty(type)) {
          ...
          i = listeners.length;
          //console.log("dispatchEvent", "listeners:", listeners);
          while (--i > -1) {
            handler = listeners[i];
            func = handler.method;
    ...
    Object.defineProperty(obj, "on", {
      value: function (type, method, params, once) {
        ...
        //console.info("on", "type:", type, "registry[type]:", registry[type]);
   ...
   * CONCRETE operation
   */
  ad.politeLoad = function (images, callback) {
    // add intern callback
    function done() {
      if (typeof callback === "function") {
        callback();
      }
      // CONCRETE operation renamed to setBkgImages because setDynamicContent is confusing
      ad.setBkgImages();
      ad.loaded = true;
      ad.start();
    }
    ...
    ad.tl = new gsap.timeline({
      ...
      onStart: function () {
        ...
        if (ad.has("started")) {
          ad.dispatchEvent({
            type: "started",
            time: ad.tl.totalTime() / ad.tl.timeScale(),
          });
        }
      },
      onRepeat: function () {
  ...
  ad.start = function () {
    ...
    console.log("EG+ 🌐 H T M L 5 - S L I M - T E M P L A T E", "3.4.0", "🔒 URI PARAMS:", gup);
    // set automatic fallback situation
    if (typeof gup.egpfallback === "string" && gup.egpfallback === "1") {
      setTimeout(function () {
        var totalDur = ad.tl.totalDuration(),
          allLabels = ad.tl.labels;
        console.log("");
        console.log("10");
        console.log("ad:", ad);
        console.log("allLabels:", allLabels);
        if (allLabels.fallback) {
          ad.tl.pause("fallback");
        } else {
          ad.tl.pause(totalDur);
        }
        if (ad.videos && ad.videos.length) {
          setTimeout(function () {
            var i, l, vid;
            l = ad.videos.length;
            for (i = 0; i < l; i++) {
  ...
  eg.createElem = function (parent, tag, attrObj, styleObj) {
    var i,
      l,
      n,
      ns = eg.nsElems.indexOf(tag) !== -1 ? "http://www.w3.org/2000/svg" : "",
  ...
  eg.initAd = function (initObj) {
    ...
    // Setting ad size
    ad.size = {
      width: gsap.getProperty($wrapper[0], "width"),
      height: gsap.getProperty($wrapper[0], "height"),
    };
  ...
  eg.clickThrough = function (e) {
    ...
    if (hasClickThrough) {
      ...
      window.open(gup.clicktag, "_blank");
    }
  ...
  function registerBtnClick($elem, eventType) {
    ...
    if ($elem && $elem.length > 0) {
      ...
      $elem[0].addEventListener(
        eventCaller,
        function (e) {
          var isCloseBtnInsideModal =
            eventType === "modalClick" &&
            (e.target.getAttribute("egp-module") === "close-btn" || e.target.tagName.toUpperCase() === "EGP-CLOSE-BTN");
          ...
          if ($target.length > 0 && eventType === "openBtnClick") {
            ...
            gsap.set('[egp-module="close-btn"], egp-close-btn', {
              css: {
                display: "block",
              },
            });
          ...
          if (eventType === "closeBtnClick") {
            ...
            if ($target.length > 0) {
              ...
              document.body.removeEventListener("click", eg.clickThrough, false);
              gsap.set($elem[0].parentNode, {
                css: {
                  display: "none",
                },
              });
              document.body.removeChild($elem[0].parentNode);
              ad = null;
              eg = null;
              e.stopPropagation();
              return false;
  ...
  function registerClickThrough() {
    ...
    if ($wrapper && $wrapper.length > 0) {
      ...
      document.body.addEventListener("click", eg.clickThrough, false);
    }
  }
