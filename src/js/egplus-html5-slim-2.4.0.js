...
(function () {
    ...
    var p, eg, ad, $, gup, $wrapper, hasRegisteredBgExit = false, initialized = false, initializedAd = false, UID = 0;
    ...
    ad.setDynamicImages = function () {
		...
		for (i = 0; i < l; i++) {
			...
			if (image.id !== '') {
                ...
                //console.log("setDynamicImages", "image.id:", image.id, "$elem[0]:", $elem[0]);
                $elem[0].style.background = 'url(' + image.url + ') ' + x + ' ' + y + '/' + bs + ' ' + r + ' transparent';
			}
			//console.log("ad.setDynamicImages ", "$elem:", $elem, "image:", image, "url:", ('url(' + image.url + ') ' + x + ' ' + y + '/' + bs + ' ' + r + ' transparent'));
		}
    };
	...
	ad.politeLoad = function (images) {
		...
		function done() {
			// CONCRETE operation
			ad.setDynamicImages();
            //console.log("ad.politeLoad done()");
            ad.loaded = true;
			ad.start();
		}
    ...
    ad.start = function () {
        ...
        //console.log("TemplatePattern ad.start call ad.setTimelines:", ad.setTimelines);
        ad.setTimelines();
        // HOOK operation - maybe overridden by client
        ...
        if (ad.has('start')) {
            //console.log("TEMPLATE method ad.start dispatch PARAMS:", gup);
        ...
        if (typeof gup.egpfallback === 'string' && gup.egpfallback === '1') {
            ...
            ad.tl.seek(ad.tl.totalDuration());
        }
    };
	...
	eg.createElem = function (parent, tag, attrObj, styleObj) {
		...
		for (n in styleObj) {
			if (styleObj.hasOwnProperty(n)) {
				//console.log("createElem ", n, styleObj[n]);
        ...
        if ($wrapper && $wrapper.length > 0) {
            ...
            $wrapper[0].addEventListener('click', function (e) {
				...
				if (hasClickThrough) {
					...
					window.open(gup.clicktag || 'https://www.example.com/', '_blank');
				}
